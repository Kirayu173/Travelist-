...
try:
    ...
    try:
        ...
        PSYCOPG_VERSION = 3
        _SIMPLE_POOL = True
        _IMPORT_NOTE = "psycopg (direct)"

        class ConnectionPool:  # type: ignore[misc]
            def __init__(self, conninfo: str, **_: Any) -> None:
        ...
        try:
            ...
            PSYCOPG_VERSION = 2
            _SIMPLE_POOL = False
            _IMPORT_NOTE = "psycopg2"
        except ImportError:
            raise ImportError(
                "Neither 'psycopg' nor 'psycopg2' library is available. "
                "Please install one of them using "
                "'pip install psycopg[pool]' or 'pip install psycopg2'"
            )

from mem0.vector_stores.base import VectorStoreBase

logger = logging.getLogger(__name__)
logger.info("Using %s for PostgreSQL connections", _IMPORT_NOTE)
...
class PGVector(VectorStoreBase):
    def __init__(
        ...
        Args:
            ...
            host: Database host.
            port: Database port.
            diskann: Whether to enable DiskANN.
            hnsw: Whether to enable HNSW index.
            minconn: Minimum connections in pool.
            maxconn: Maximum connections in pool.
            sslmode: PostgreSQL SSL mode (e.g. 'require').
            connection_string: Optional DSN overriding individual params.
            connection_pool: Pre-configured connection pool.
        """
        ...
        # Connection priority: pool > connection_string > individual parameters
    ...
    def _get_cursor(self, commit: bool = False):
        """
        Unified context manager yielding a cursor from the active pool.
        Automatically commits/rolls back and returns the connection.
        """
        if PSYCOPG_VERSION == 3:
            ...
            with self._get_cursor(commit=True) as cur:
                cur.executemany(
                    (
                        ...
                        "(id, vector, payload) VALUES (%s, %s, %s)"
                    ),
                    data,
                )
        else:
                ...
                SELECT
                    ...
                    (
                        SELECT pg_size_pretty(
                            pg_total_relation_size('{self.collection_name}')
                        )
                    ) as total_size
