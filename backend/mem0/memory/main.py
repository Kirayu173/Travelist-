...
class Memory(_MemoryHelpersMixin, MemoryBase):
    def __init__(self, config: MemoryConfig | None = None):
        ...
        if hasattr(self.config.vector_store.config, "model_dump"):
            ...
            for attr in [
                ...
                if hasattr(self.config.vector_store.config, attr):
                    telemetry_config_dict[attr] = getattr(
                        self.config.vector_store.config, attr
                    )
        ...
        telemetry_config = self.config.vector_store.config.__class__(
            **telemetry_config_dict
        )
        ...
        *,
        ...
        if memory_type is not None and memory_type != MemoryType.PROCEDURAL.value:
            raise Mem0ValidationError(
                ...
                details={
                    ...
                    "valid_type": MemoryType.PROCEDURAL.value,
                },
        ...
        if isinstance(messages, str):
            ...
            raise Mem0ValidationError(
                ...
                details={
                    "provided_type": type(messages).__name__,
                    "valid_types": ["str", "dict", "list[dict]"],
                },
    ...
    def _add_to_vector_store(self, messages, metadata, filters, infer):
        ...
        if self.config.custom_fact_extraction_prompt:
            ...
            is_agent_memory = self._should_use_agent_memory_extraction(
                messages, metadata
            )
            ...
            for resp in new_memories_with_actions.get("memory", []):
                ...
                try:
                    ...
                    if event_type == "ADD":
                        ...
                        if memory_id and (
                            metadata.get("agent_id") or metadata.get("run_id")
                        ):
        ...
        capture_event(
            ...
            self,
            {
                ...
                "advanced_filters": bool(
                    filters and self._has_advanced_operators(filters)
                ),
            },
        )
                    ...
                    if event_type == "ADD":
                        ...
                        if memory_id and (
                            ...
                            async def update_session_ids(mem_id, meta):
                                ...
                                logger.info(
                                    f"Updated session IDs for memory {mem_id} (async)"
                                )

                            task = asyncio.create_task(
                                update_session_ids(memory_id, metadata)
                            )
